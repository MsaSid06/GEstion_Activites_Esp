...
function renderCalendar() {
    ...
    const formattedDate = currentDate.toLocaleDateString("fr-FR", { month: "long", year: "numeric" });
    ...
    const datesAvenir = DataActivites
    ...
    const datesEnCours = DataActivites
...
function renderAll() {
    ...
    document.getElementById("counter-avenir").innerText = DataActivites.filter(a => a.statut === "À venir").length;
    document.getElementById("counter-encours").innerText = DataActivites.filter(a => a.statut === "En cours").length;
    document.getElementById("counter-termine").innerText = DataActivites.filter(a => a.statut === "Terminé").length;
    document.getElementById("activities-count").innerText = `${filtered.length} activité(s) trouvée(s)`;
...
function filterDepartment(dp) { currentDept = dp; renderAll(); }
function searchActivities(v) { searchQuery = v; renderAll(); }
...
function openDetailedView(id) {
    ...
    if (act.statut === "À venir") {
        badge.className = "bg-purple-500/20 backdrop-blur-md text-white font-bold text-xs px-3.5 py-1 rounded-full flex items-center gap-1.5 border border-purple-400/30";
    } else if (act.statut === "En cours") {
        badge.className = "bg-amber-500/20 backdrop-blur-md text-white font-bold text-xs px-3.5 py-1 rounded-full flex items-center gap-1.5 border border-amber-400/30";
    } else {
        badge.className = "bg-emerald-500/20 backdrop-blur-md text-white font-bold text-xs px-3.5 py-1 rounded-full flex items-center gap-1.5 border border-emerald-400/30";
    }
