...
<body class="bg-light-gray min-h-screen font-sans antialiased pb-32">
    ...
    <header class="bg-[#650665] px-6 py-5 flex justify-between items-center sticky top-0 z-40 shadow-md">
        ...
        <div class="flex items-center gap-6 relative">
            <button onclick="toggleNotifications(event)"
                class="relative text-white/90 hover:text-white transition focus:outline-none">
                <svg xmlns="[w3.org](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" stroke-width="2"
        ...
        <section id="view-dashboard" class="space-y-8 block">
            <div>
                ...
                <p class="text-sm text-gray-500 font-medium mt-1">
                    Activités de l'ESP - <span id="dash-subtitle-date">Juin 2026</span>
                </p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white rounded-2xl shadow-sm border-l-[6px] border-[#0047ab] px-6 py-5 flex items-center gap-3">
                ...
                <div class="bg-white rounded-2xl shadow-sm border-l-[6px] border-[#ff7a00] px-6 py-5 flex items-center gap-3">
                ...
                <div class="bg-white rounded-2xl shadow-sm border-l-[6px] border-[#16a34a] px-6 py-5 flex items-center gap-3">
            ...
            <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                <div class="md:col-span-4 relative flex items-center">
                    <span class="absolute left-4 text-gray-600">
                        <svg xmlns="[w3.org](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" stroke-width="2.5"
                            stroke="currentColor" class="w-4 h-4">
                            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <select id="structure-filter" onchange="filterDepartment(this.value)"
        ...
        <option value="ALL">Toutes les structures</option>
                ...
                <div class="md:col-span-4 flex gap-1 justify-end">
                    <button onclick="filterGrid('ALL', this)"
                        class="grid-filter-btn bg-esp-purple text-white px-5 py-2.5 rounded-full text-xs font-bold shadow-sm">Tout</button>
                    <button onclick="filterGrid('AVENIR', this)"
                        class="grid-filter-btn bg-white border border-gray-200 text-gray-600 px-5 py-2.5 rounded-full text-xs font-semibold hover:bg-gray-50">À venir</button>
                    <button onclick="filterGrid('EN_COURS', this)"
                        class="grid-filter-btn bg-white border border-gray-200 text-gray-600 px-5 py-2.5 rounded-full text-xs font-semibold hover:bg-gray-50">En cours</button>
                    <button onclick="filterGrid('TERMINE', this)"
    ...
    <div class="fixed left-1/2 -translate-x-1/2 z-50" style="bottom:18px">
        <div class="flex items-center"
            style="background:#650665;border-radius:999px;padding:8px 14px;gap:6px;box-shadow:0 12px 34px rgba(40,8,40,.35)">
            <button id="btn-dock-dash" onclick="switchView('dashboard')"
                class="flex items-center justify-center transition-all"
                style="width:46px;height:46px;border-radius:50%;background:rgba(255,255,255,.18);color:#fff">
                <svg xmlns="[w3.org](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" stroke-width="1.8"
            ...
            <button id="btn-dock-list" onclick="switchView('list')"
                class="flex items-center justify-center transition-all"
                style="width:46px;height:46px;border-radius:50%;color:rgba(255,255,255,.72)">
                <svg xmlns="[w3.org](http://www.w3.org/2000/svg)" fill="none" viewBox="0 0 24 24" stroke-width="1.8"
                    stroke="currentColor" style="width:22px;height:22px">
                    <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <a href="../profil.php" title="Mon profil" class="relative flex items-center justify-center"
                style="width:46px;height:46px;border-radius:50%;background:#322336;color:#fff;font-weight:800;font-size:14px">
                <?= strtoupper(substr($_SESSION['prenom'], 0, 1) . substr($_SESSION['nom'], 0, 1)) ?>
                <span class="absolute"
                    style="bottom:2px;right:2px;width:9px;height:9px;border-radius:50%;background:#D4AF37;border:2px solid #650665"></span>
            </a>
        </div>
    </div>
