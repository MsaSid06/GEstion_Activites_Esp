...
$init   =  strtoupper($_SESSION['prenom'][0] . $_SESSION['nom'][0]) ;
...
<style>
    ...
    .gnav .gnav-center svg {
        width: 26px;
        height: 26px;
    }

    .gnav .gnav-center:hover {
        background: #f3e7f3;
    }
...
<nav class="gnav" aria-label="Navigation administration">
    <?php foreach ($items as $it):
        ...
        if (empty($it['center']) && ($it['page'] ?? '') === $page) {
            $cls .= ' active';
        }
        ?>
    <a class="<?= $cls ?>"
    ...
    <a class="gnav-avatar" href="<?= $BASE ?>/profil.php"
        title="Mon profil">
